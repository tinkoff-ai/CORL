...
last_offline_scores, last_offline_stds = get_last_scores(avg_offline_scores, avg_offline_stds)
...
last_online_scores, last_online_stds = get_last_scores(avg_online_scores, avg_online_stds)
...
def get_table(scores, stds, pm="$\\pm$", delim=" & ", row_delim="\\midrule", row_end=" \\\\", row_begin="",
              scores2=None, stds2=None, scores_delim=" $\\to$ "):
    rows = [
        ...
        for algo in algorithms:
            if data in stds[algo]:
                row.append(f"{scores[algo][data]:.2f} {pm} {stds[algo][data]:.2f}" + (
                    "" if scores2 is None else f"{scores_delim} {scores2[algo][data]:.2f} {pm} {stds2[algo][data]:.2f}"))
            else:
                row.append(f"{scores[algo][data]:.2f}" + (
                    "" if scores2 is None else f"{scores_delim} {scores2[algo][data]:.2f}"))
